...
url.searchParams.append('topic', 'http://localhost:8000/books/{id}'); // sujet à écouter
...
const es = new EventSource('http://localhost:3000/hub?topic=' + encodeURIComponent('http://localhost:8000/books/{id}'));
es.onmessage = e => {
...
document.querySelector("#getBook").addEventListener('click', e => {
    ...
    fetch('http://localhost:8000/books/'+id, init);
});